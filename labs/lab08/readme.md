...
```
Также возвращаемся на R1 в интерфейс g0/0/1.1000 и указываем там ту же инкапсуляцию (соответствие по обе стороны trunk)
```
...
Ответ:<br>
Какой-то свободный адрес из пула адресов заданного диапазона, обусловленного настройками DHCP-сервера, исключая зарезервированные (шлюз и бродкаст).
...
```
R1(config)#ip dhcp pool subweb_A
```
