...
1.	Почему обоим интерфейсам Ethernet на R1 можно назначить один и тот же локальный адрес канала — FE80::1?<br>
Адрес link-local не маршрутизируется. Две, условно, подсети не пересекаются. Одна сеть с PC-B, вторая с коммутатором S1. Обе могут использовать один и тот же link-local.
3.	Какой идентификатор подсети в индивидуальном IPv6-адресе 2001:db8:acad::aaaa:1234/64?<br>
Т.к. префикс /64, сетевая часть занимает четыре первых «хекстета»/блока по 16 бит - 2001:db8:acad:0 (в примере :: - это сокращение нулей стоящих подряд в адресе). Значит идентификатор подсети в индивидуальном IPv6-адресе 2001:db8:acad::aaaa:1234/64 будет 0000 (сокращенной записью  - 0)
