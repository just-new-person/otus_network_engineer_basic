...
При настройке маршрутизатора я установил вторую версию SSH (ip ssh version 2), по уроку. В методичке не было указано ставить, но рекомендоавлось из-за уязвимости на уроке.<br>
А вот принастройке роутера я забыл об этом (настраивал на следующий день), однако подключиться по IP так же прекрасно получилось.<br>
Вывод: судя по всему поддерживаются оба протокола - 1.99 и 2.<br>

Закройте окно настройки.
	Вопрос для повторения
Как предоставить доступ к сетевому устройству нескольким пользователям, у каждого из которых есть собственное имя пользователя?<br>
Добавить в локальную базу несколько учетных записей, указав им уровень прав (привелегий) и пароли.
